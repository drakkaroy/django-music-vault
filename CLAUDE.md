...
- **Frontend (React rewrite)**: lives in `frontend/`, served in *parallel* with the vanilla app at `/react/` — not a replacement yet, don't remove the vanilla frontend until the React version has full feature parity. Its built output (`music_vault/static/music_vault/react-app/`) is committed — always `npm run build` (from `frontend/`) and commit the result alongside a source change there, the same way you wouldn't leave the Python side with an uncommitted migration. No React Router, no state-management library, no CSS framework — plain Context/hooks and plain CSS with the same custom properties as the vanilla app, matching this project's "nada extra" dependency discipline. No browser is available in this environment — `npm run smoke` (jsdom) is the closest verification to an actual render check; treat unverified UI as unverified until someone checks it in a real browser. Details: [docs/frontend.md#react-rewrite-in-progress](docs/frontend.md#react-rewrite-in-progress).

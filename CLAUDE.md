...
- **Frontend (React rewrite)**: lives in `frontend/`, **the default UI, served at `/`** (`music_vault:vault` → `views.vault_react`). The original vanilla frontend is kept at `/legacy/` (`music_vault:vault-legacy` → `views.vault`) for reference/rollback, not deleted — see [docs/frontend.md](docs/frontend.md) for what that name swap did and didn't require touching (`window.MV_BASE`, `LOGIN_REDIRECT_URL`, the Spotify OAuth redirect-back all kept working because the URL name `vault` stayed attached to the `""` path). No real browser has verified the UI yet (this environment has none — `npm run smoke` against a `jsdom`-mounted build is the closest substitute, not equivalent) — factor that in before treating a UI change as fully verified. Its built output (`music_vault/static/music_vault/react-app/`) is committed — always `npm run build` (from `frontend/`) and commit the result alongside a source change there, the same way you wouldn't leave the Python side with an uncommitted migration. No React Router, no state-management library, no CSS framework — plain Context/hooks and plain CSS with the same custom properties as the vanilla app, matching this project's "nada extra" dependency discipline. **Run `npm run format` (Prettier) before every commit** — the user explicitly asked that all new frontend code stay formatted/readable, not just functional. New frontend-only features (not ported to the vanilla app, which is frozen) are expected going forward — e.g. the covers-only album view (`ViewToggle`). Known simplifications vs. the vanilla app (cosmetic only, not data-affecting): no per-library filter memory, no toast fade-out animation, no `/`-to-focus-search shortcut. Details: [docs/frontend.md#react-rewrite](docs/frontend.md#react-rewrite).
